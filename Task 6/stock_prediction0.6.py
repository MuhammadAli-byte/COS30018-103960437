...
print("Finding the best SARIMA parameters using auto_arima...")
...
print(f"Best SARIMA parameters: order={best_order}, seasonal_order={best_seasonal_order}")
...
# Combine DL and SARIMA predictions using weighted average
weight_dl = 0.7
weight_sarima = 0.3
...
mae_ensemble = mean_absolute_error(actual_prices_flat, ensemble_predictions_flat)
rmse_ensemble = np.sqrt(mean_squared_error(actual_prices_flat, ensemble_predictions_flat))
...
plt.plot(actual_prices_flat, label='Actual Prices', color='black')
plt.plot(predicted_prices_flat, label='DL Predictions', linestyle='dashed', color='green')
plt.plot(ensemble_predictions_flat, label='Ensemble Predictions', linestyle='dashed', color='red')
plt.xlabel('Time')
plt.ylabel('Stock Price')
plt.title('Stock Price Prediction - Ensemble Model')
plt.legend()
plt.show()
